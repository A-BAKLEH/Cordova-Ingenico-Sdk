...
public class BytesUtil {
    ...
    public static int ascToInt(byte sum) {
        return Integer.valueOf(Character.toString((char) (sum & 0xFF)));
    }
     ...
     * @return 字节数组
     */
    public static byte[] toBytes(String data, Charset charsetName) {
        if (isNullEmpty(data)) {
            return EMPTY_BYTE_ARRAY;
        }
        try {
     ...
     * @return 字符串
     */
    public static String fromBytes(byte[] data, Charset charsetName) {
        if (isNullEmpty(data)) {
            return EMPTY_STRING;
        }
        try {
